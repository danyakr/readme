...
##### 7.2.2 Фикстура open_login_page

Фикстура выполняет перемещение на страницу авторизации перед запуском каждого теста, где эта фикстура используется. Используется как параметр функции теста, например, `def test_login_btn_exists(self, login_page, open_page)`.
